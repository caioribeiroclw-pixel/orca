...
describe('NewWorkspaceComposerCard folder task source mode', () => {
  ...
  it('shows the setup startup policy toggle only when setup is available', () => {
    ...
    expect(current.container.textContent).not.toContain('Wait for setup before starting agent')
    ...
    expect(current.container.textContent).toContain('Wait for setup before starting agent')
  })
  ...
  it('emits the setup startup policy toggle value', () => {
    ...
    const waitLabel = [...current.container.querySelectorAll('label')].find((label) =>
      label.textContent?.includes('Wait for setup before starting agent')
    )
    const checkbox = waitLabel?.querySelector<HTMLElement>('[role="checkbox"]')
    expect(checkbox).toBeTruthy()
    act(() => checkbox?.click())
